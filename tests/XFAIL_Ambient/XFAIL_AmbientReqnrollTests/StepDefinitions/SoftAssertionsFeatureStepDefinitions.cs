...
namespace XFAIL_AmbientReqnrollTests.StepDefinitions
{
    [Binding]
    public class StatefulAssertionsFeatureStepDefinitions(StatefulAsserter assert, ScenarioContext ctx)
    {
        ...
        {
            _ = ctx.Get<AggregateAssertionException>("FlushException") ?? throw new Exception("No flush exception found");
            await Stateless.Assert.That(GetAsserter().Soft.SoftState.AlreadyFlushed).IsTrue();
        }


        [Then("the test will fail")]
        public static void ThenTheTestWillFail()
        {
        ...
        [When("I use hard asserter that throws")]
        public async Task WhenIUseHardAsserterThatThrows()
        {
            Thread.Sleep(100);
            try
            {
                await GetAsserter().That(5).Is(6);
            }
            catch (AssertionException)
            {
                // Expected - hard assertion throws immediately
            }
            Thread.Sleep(100);
        }
